...
@objc public final class LanguageConvertor: NSObject {
    ...
    private func localValue(_ key: String) -> String {
        ...
        if !CallAppearance.call_language.rawValue.isEmpty {
            lang = CallAppearance.call_language.rawValue
        }
        
        let path = Bundle.callBundle.path(forResource: lang, ofType: "lproj") ?? ""
    ...
    static func chineseLanguage() -> Bool {
        if CallAppearance.call_language.rawValue.contains("zh") {
            return true
        } else {
            return false
        }
